...
# ---------- UI Execution ----------
st.set_page_config(page_title="Pro Batch Image Processor", layout="wide")
st.title("Pro Batch Image Processor")

IMG_DATA_KEY = "img_data_full"
DOWNSCALE_KEY = "downscale_size_full"
...
with st.sidebar:
    ...
    if st.session_state[IMG_DATA_KEY]:
        ...
        active_file = st.selectbox("Pick image to edit:", list(st.session_state[IMG_DATA_KEY].keys()))
        ...
        if st.button("🚀 Process & Download All (ZIP)", type="primary"):
            ...
            with zipfile.ZipFile(zip_buffer, "w") as zip_file:
                for idx, (fname, d) in enumerate(st.session_state[IMG_DATA_KEY].items()):
                    ...
                    prog.progress((idx + 1) / len(st.session_state[IMG_DATA_KEY]))
...
# ---------- Main Panel ----------
if st.session_state[IMG_DATA_KEY] and 'active_file' in locals():
    data = st.session_state[IMG_DATA_KEY][active_file]
